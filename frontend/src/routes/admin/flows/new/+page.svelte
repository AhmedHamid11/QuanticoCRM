<script lang="ts">
	...
	import type { FlowStep, FlowBuilderDefinition, FlowCreateInput, ScreenStep, DecisionStep, AssignmentStep, RecordCreateStep, RecordUpdateStep, RecordGetStep, WebhookStep, EndStep } from '$lib/types/flow';
	...
	async function handleSubmit() {
		...
		// Build definition
		const definition: FlowBuilderDefinition = {
			trigger: {
				type: triggerType,
		...
		try {
			...
			await post('/flows', input);
			toast.success('Flow created');
			goto('/admin/flows');
		} catch (e) {
			const message = e instanceof Error ? e.message : 'Failed to create flow';
			toast.error(message);
		} finally {
			saving = false;
		}
...
<div class="max-w-6xl mx-auto space-y-6">
	<div class="flex justify-between items-center">
		<div>
			<h1 class="text-2xl font-bold text-gray-900">New Screen Flow</h1>
			<p class="text-sm text-gray-500 mt-1">Create an interactive wizard with screens, decisions, and actions</p>
		</div>
		<a
			href="/admin/flows"
