<script lang="ts">
	...
	import type { Quote, QuoteCreateInput, QuoteLineItemInput } from '$lib/types/quote';
	...
	onMount(async () => {
		try {
			lineItemFields = await get<FieldDef[]>('/entities/QuoteLineItem/fields');
		} catch (err) {
			console.warn('Failed to load QuoteLineItem field definitions, using defaults');
		}
	});
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			const quote = await post<Quote>('/quotes', input);
...
<div class="max-w-4xl mx-auto">
	<div class="flex items-center justify-between mb-6">
		...
		<a href="/quotes" class="text-gray-600 hover:text-gray-900 text-sm">
			← Back to Quotes
		</a>
	</div>
	...
	<form onsubmit={handleSubmit} class="space-y-6">
		...
		<div class="flex justify-end gap-3">
			<a href="/quotes" class="px-4 py-2 text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">
				Cancel
			</a>
