<script lang="ts">
	...
	async function loadLayout() {
		try {
			fieldDefs = await get<FieldDef[]>(`/entities/Account/fields`);

			// Load layout
			try {
				const layoutData = await get<{ layoutData: string }>(`/entities/Account/layouts/list`);
				const parsed = JSON.parse(layoutData.layoutData || '[]');
				layoutFields = Array.isArray(parsed) ? parsed : [];
			} catch {
				layoutFields = [];
			}

			// Default columns if no layout configured
			if (layoutFields.length === 0) {
				layoutFields = ['name', 'industry', 'phoneNumber', 'website'];
			}
		} catch (e) {
	...
	async function loadListViews() {
		try {
			listViews = await get<ListView[]>(`/list-views/Account`);
	...
	function getFieldDef(fieldName: string): FieldDef | undefined {
		return fieldDefs.find((f) => f.name === fieldName);
	}
	...
	function formatFieldValue(record: Account, fieldName: string): string {
		const field = getFieldDef(fieldName);
		let value: unknown;
	...
	function isSortableField(fieldName: string): boolean {
		// Common sortable fields
		const sortable = ['name', 'createdAt', 'created_at', 'modifiedAt', 'modified_at'];
		return sortable.includes(fieldName);
	}
	...
	async function loadAccounts() {
		try {
			...
			const result = await get<AccountListResponse>(`/accounts?${params}`);
			accounts = result.data;
			total = result.total;
			totalPages = result.totalPages;
			knownTotal = result.total;
		} catch (e) {
			const errorMsg = e instanceof Error ? e.message : 'Failed to load accounts';
	...
	async function saveListView() {
		...
		try {
			if (saveMode === 'update' && viewToUpdate) {
				...
				const updatedView = await put<ListView>(`/list-views/Account/${viewToUpdate.id}`, {
				...
				addToast('List view updated', 'success');
			} else {
				// Create new view
				if (!newViewName.trim()) return;
				const newView = await post<ListView>(`/list-views/Account`, {
				...
				addToast('List view saved', 'success');
			}
			showSaveModal = false;
			newViewName = '';
			saveAsDefault = false;
			...
			addToast(e instanceof Error ? e.message : 'Failed to save list view', 'error');
		} finally {
			savingView = false;
		}
	}
	...
	async function deleteListView(view: ListView) {
		if (view.isSystem) {
			addToast('Cannot delete system views', 'error');
			return;
		}
		...
		try {
			await del(`/list-views/Account/${view.id}`);
			addToast('List view deleted', 'success');
		} catch (e) {
			listViews = backup;
			addToast(e instanceof Error ? e.message : 'Failed to delete list view', 'error');
		}
	}

	async function setDefaultView(view: ListView) {
		try {
			await put<ListView>(`/list-views/Account/${view.id}`, {
				name: view.name,
				filterQuery: view.filterQuery,
			...
			await loadListViews();
			addToast('Default view updated', 'success');
		} catch (e) {
			addToast(e instanceof Error ? e.message : 'Failed to set default view', 'error');
		}
	}
...
<div class="space-y-4">
		...
		<div class="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-3">
			...
			<div class="relative">
				<input
					...
					placeholder="e.g., industry = 'Technology' AND name LIKE '%Corp%'"
			...
			<div class="text-xs text-gray-500 space-y-1">
				...
				<p><strong>Examples:</strong> <code class="bg-gray-200 px-1 rounded">name LIKE '%Tech%'</code> | <code class="bg-gray-200 px-1 rounded">industry IN ('Technology', 'Finance')</code></p>
			</div>
		</div>
	{/if}
	...
	{#if loading || !layoutLoaded}
		<TableSkeleton rows={pageSize} columns={layoutFields.length || 4} />
	{:else if error}
		<ErrorDisplay message={error} onRetry={loadAccounts} />
	{:else if layoutFields.length === 0}
		<div class="text-center py-12">
			<p class="text-gray-500 mb-4">No columns configured for the list view.</p>
			<a
				href="/admin/entity-manager/Account/layouts/list"
				class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-black bg-primary hover:bg-primary/90"
			>
	...
	{:else if accounts.length === 0}
		<div class="text-center py-12 text-gray-500">
			...
			{:else}
				No accounts found. <a href="/accounts/new" class="text-primary hover:underline">Create one</a>
			{/if}
		</div>
	{:else}
				...
				<thead class="bg-gray-50">
					<tr>
						{#each layoutFields as fieldName}
							<th
								class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider {isSortableField(fieldName) ? 'cursor-pointer hover:bg-gray-100' : ''}"
								onclick={() => isSortableField(fieldName) && handleSort(fieldName)}
							>
				...
				<tbody class="bg-white divide-y divide-gray-200">
					{#each accounts as account (account.id)}
						<tr class="hover:bg-gray-50">
							{#each layoutFields as fieldName}
								<td class="px-6 py-4 whitespace-nowrap text-sm {isNameField(fieldName) ? 'font-medium' : 'text-gray-500'}">
									{#if isNameField(fieldName)}
										<a href="/accounts/{account.id}" class="text-primary hover:underline">
											{formatFieldValue(account, fieldName)}
										</a>
									{:else if isLinkField(fieldName)}
										{@const value = (account as Record<string, unknown>)[fieldName]}
										{#if value}
											<a href={String(value)} target="_blank" class="text-primary hover:underline">
												{formatFieldValue(account, fieldName)}
											</a>
										{:else}
											-
		...
		{#if totalPages > 1}
			<div class="flex justify-between items-center">
				...
				<div class="flex gap-2">
					<button
						onclick={() => { page = page - 1; loadAccounts(); }}
						disabled={page === 1}
				...
				{#if saveMode === 'new'}
					<div>
						...
						<input
							...
							placeholder="e.g., Technology Companies, Large Accounts"
