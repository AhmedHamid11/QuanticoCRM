...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="space-y-6"><div><nav class="text-sm text-gray-500 mb-2"><a${attr("href", `/${stringify(
      ...
      // Load data
      entitySlug
    )}`)} class="hover:text-gray-700">${escape_html(entityName + "s")}</a> <span class="mx-2">/</span> <span class="text-gray-900">New</span></nav> <h1 class="text-2xl font-bold text-gray-900">New ${escape_html(entityName)}</h1></div> `);
    {
      $$renderer2.push("<!--[-->");
