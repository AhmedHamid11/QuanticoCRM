...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="space-y-6"><nav class="text-sm text-gray-500 mb-2"><a href="/admin" class="hover:text-gray-700">Administration</a> <span class="mx-2">/</span> <a href="/admin/entity-manager" class="hover:text-gray-700">Entity Manager</a> <span class="mx-2">/</span> <a${attr("href", `/admin/entity-manager/${stringify(entityName)}`)} class="hover:text-gray-700">${escape_html(entityName)}</a> <span class="mx-2">/</span> <span class="text-gray-900">Validation Rules</span></nav> <div class="flex justify-between items-center"><div><h1 class="text-2xl font-bold text-gray-900">Validation Rules</h1> <p class="text-sm text-gray-500 mt-1">Rules that validate ${escape_html(entityName)} records before save operations</p></div> <a${attr("href", `/admin/entity-manager/${stringify(entityName)}/validation-rules/new`)} class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-600/90">+ New Rule</a></div> `);
    {
      $$renderer2.push("<!--[-->");
