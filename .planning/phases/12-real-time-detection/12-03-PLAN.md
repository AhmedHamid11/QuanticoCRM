...
Note: Uses existing `get` and `post` functions from `$lib/utils/api`. The 404 handling in getPendingAlert treats "no alert" as a normal case (returns null) rather than an error. The `isBlockMode` field is included in PendingAlert type - it comes from the matching rule configuration and is populated by the backend when the alert is created.
  </action>
  <verify>
  ...
  <done>
    ...
    TypeScript types for all dedup entities including isBlockMode field.
...
- Shows "Block Mode" badge when alert.isBlockMode is true
...
- Responsive layout with flex-wrap for small screens
  ...
  <done>
    ...
    Block mode indicator shown when applicable.
...
- Block mode: requires typing "DUPLICATE" to proceed (isBlockMode prop from alert)
...
- DuplicateAlertBanner shows "Block Mode" badge when isBlockMode is true
...
- Block mode support with "DUPLICATE" override (triggered by isBlockMode prop)
- All TypeScript types correct including isBlockMode in PendingAlert
