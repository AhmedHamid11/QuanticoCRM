...
Note: `DuplicateAlertMatch` includes `RecordName` for display in UI. The `MatchResult` type already exists in `entity/dedup.go`.
  </action>
  <verify>
  ...
  <done>
    Migration file creates pending_duplicate_alerts table with proper indexes.
...
func (r *PendingAlertRepo) Upsert(ctx context.Context, alert *entity.PendingDuplicateAlert) error {
    ...
    query := `
        ...
        (id, org_id, entity_type, record_id, matches_json, total_match_count,
         highest_confidence, detected_at, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `
    ...
    _, err = r.db.ExecContext(ctx, query,
        ...
        alert.DetectedAt.Format(time.RFC3339), alert.Status)
...
func (r *PendingAlertRepo) GetPendingByRecord(ctx context.Context, orgID, entityType, recordID string) (*entity.PendingDuplicateAlert, error) {
    query := `
        SELECT id, org_id, entity_type, record_id, matches_json, total_match_count,
               highest_confidence, detected_at, status, resolved_at, resolved_by_id, override_text
    ...
    err = rows.Scan(
        ...
        &detectedAt, &alert.Status, &resolvedAt, &resolvedByID, &overrideText,
    )
    if err != nil {
        return nil, err
    }
...
<success_criteria>
- Migration creates pending_duplicate_alerts table with indexes
- PendingDuplicateAlert entity type with proper JSON/DB tags
- PendingAlertRepo with Upsert, GetPendingByRecord, Resolve methods
- GET /dedup/:entity/:id/pending-alert endpoint returns alert or 404
