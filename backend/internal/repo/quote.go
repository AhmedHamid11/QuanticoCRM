...
func (r *QuoteRepo) GetByID(ctx context.Context, orgID, id string) (*entity.Quote, error) {
	...
	query := `
		SELECT q.id, q.org_id, COALESCE(q.name, ''), COALESCE(q.quote_number, ''), COALESCE(q.status, ''),
			...
			COALESCE(q.description, ''), COALESCE(q.terms, ''), COALESCE(q.notes, ''), COALESCE(q.assigned_user_id, ''),
			COALESCE(q.created_by_id, ''), COALESCE(q.modified_by_id, ''), COALESCE(q.created_at, ''), COALESCE(q.modified_at, ''), COALESCE(q.deleted, 0),
